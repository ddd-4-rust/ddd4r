...
| RBatis 扩展族 | 进行中 | 独立仓库均固定 revision；rbatis-plus 原生 Mapper 已完成事务化批量更新/混合 Upsert 及 fail-closed 安全装配，并接通 SQL 重写/参数变换/结果验签/解密/观测；SQLite 回滚、安全管线和 Outbox 契约通过；SM4/SM3、四数据库及 Redis Cluster/Sentinel 拓扑仍未完成 |
